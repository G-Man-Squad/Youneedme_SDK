...
void CNPC_Combine_Armored::Spawn(void)
{
	...
	SetThink( &CNPC_Combine_Armored::SpeedThink );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Combine_Armored::SpeedThink(void)
{
	if (m_flGroundSpeed > 10)
	{
		m_flGroundSpeed = 10;
	}
...
void CNPC_Combine_Armored::Precache()
{
	if (!GetModelName())
	{
		SetModelName(MAKE_STRING("models/combine_armored_soldier.mdl"));
	}
